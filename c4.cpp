...
void SET_WORD(byte *l, WORD v) {
  *l = (v & 0xff);
  *(l + 1) = (byte)(v >> 8);
}
void SET_LONG(byte *l, long v) {
  SET_WORD(l, v & 0xFFFF);
...
void WComma(CELL v) {
  SET_WORD(&code[HERE], (WORD)v);
  HERE += 2;
}
void Comma(CELL v) {
  SET_LONG(&code[HERE], v);
  HERE += CELL_SZ;
}
...
void fCreate() {
  ...
  if (doFindInternal(word)) {
    printStringF("-redef:[%s]-", word);
  }
  LAST -= sizeof(DICT_E);
  DICT_E *dp = (DICT_E *)&st.bytes[LAST];
  dp->xt = (USHORT)HERE;
...
void fStrOps() {
  ...
  case '=':
    ...
    break; // STR-EQ
  case 'i':
    NOS = strEqI(CTOS, CNOS);
    DROP1;
    break; // STR-EQI
  case 'l':
    TOS = (CELL)strLen(CTOS);
    break; // STR-LEN
  case 'r':
    TOS = (CELL)rTrim(CTOS);
    break; // STR-RTRIM
...
void (*q[128])() = {
    X,       fBLit,    fWLit,   X,           fLit,    X,        X,
    X,       X,        X,       N,           X,       X,        N,
    X,       X,        X,       X,           X,       X,        X,
    X,       X,        X,       X,           X,       X,        X,
    X,       X,        X,       X, //   0:31
    N,       fStore,   fTypeF1, fDup,        fSwap,   fOver,    fSlashMod,
    fBLit,   N,        N,       fMult,       fAdd,    fEmit,    fSub,
    fDot,    fDiv, //  32:47
    fNum,    fNum,     fNum,    fNum,        fNum,    fNum,     fNum,
    fNum,    fNum,     fNum,    fCall,       fRet,    fLt,      fEq,
    fGt,     fIf, //  48:63
    fFetch,  X,        X,       fCharOp,     fDec,    fExecute, fFloat,
    fGoto,   X,        fIndex,  fIndex2,     fKey,    X,        fMod,
    X,       fCommaOp, //  64:79
    fInc,    X,        fRetOps, fStrOps,     fType,   X,        X,
    X,       X,        X,       fTypeF2,     fDo,     fDrop,    fLoop,
    fLeave,  fNegate, //  80:95
    fZQuote, fAbs,     fBitOp,  fLocIncCell, fLocDec, X,        fFileOp,
    X,       X,        fLocInc, X,           X,       fLocAdd,  fLocRem,
    X,       X, //  96:111
    X,       X,        fLocGet, fLocSet,     fTypeQ,  fUser,    fVarAddr,
    fWordOp, fExt,     X,       X,           fBegin,  fSQuote,  fWhile,
    fLNot,   X}; // 112:127
...
int doLiteral(int t) {
  ...
  if (t == 'v') {
    CComma('v');
    Comma(num);
  } else if (BTW(num, 0, 0xFF)) {
    CComma(1);
    CComma(num);
  } else if (BTW(num, 0, 0xFFFF)) {
    CComma(2);
    WComma(num);
  } else {
    CComma(4);
...
int doPrim(const char *wd) {
  ...
  if (BTW(vml[0], '0', '9') && BTW(code[HERE - 1], '0', '9')) {
    CComma(' ');
  }
  for (int j = 0; vml[j]; j++) {
    CComma(vml[j]);
...
void systemWords() {
  ...
  sprintF(cp, ": code-sz %d ;", CODE_SZ);
  doParse(cp);
  sprintF(cp, ": dict-sz %d ;", DICT_SZ);
  doParse(cp);
  sprintF(cp, ": mem-sz %d ;", MEM_SZ);
  doParse(cp);
  sprintF(cp, ": vars-sz %d ;", VARS_SZ);
  doParse(cp);
  sprintF(cp, ": mem %lu ;", (UCELL)&MEM[0]);
  doParse(cp);
  sprintF(cp, ": cb %lu ;", (UCELL)code);
  doParse(cp);
  sprintF(cp, ": vb %lu ;", (UCELL)vars);
  doParse(cp);
  sprintF(cp, ": (here) %lu ;", (UCELL)&HERE);
  doParse(cp);
  sprintF(cp, ": (ohere) %lu ;", (UCELL)&oHERE);
  doParse(cp);
  sprintF(cp, ": (last) %lu ;", (UCELL)&LAST);
  doParse(cp);
  sprintF(cp, ": (vhere) %lu ;", (UCELL)&VHERE);
  doParse(cp);
  sprintF(cp, ": (lex) %lu ;", (UCELL)&lexicon);
  doParse(cp);
  sprintF(cp, ": base %lu ;", (UCELL)&BASE);
  doParse(cp);
  sprintF(cp, ": >in %lu ;", (UCELL)&in);
  doParse(cp);
  sprintF(cp, ": CELL %lu ;", (UCELL)CELL_SZ);
  doParse(cp);
}
